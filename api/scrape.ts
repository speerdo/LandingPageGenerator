...
export default async function handler(req: NextRequest) {
  ...
  try {
    ...
    const scraperBeeUrl = `https://app.scraperbee.com/api/v1/?api_key=${SCRAPER_BEE_API_KEY}&url=${encodeURIComponent(url)}&javascript=true&render_js=true`;
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (!response.ok) {
          throw new Error(`ScraperBee API error: ${response.status} ${response.statusText}`);
        }

        const data = await response.json();
        
        if (data.error) {
          throw new Error(`ScraperBee returned error: ${data.error}`);
        }

        const html = data.body || data.html || '';
        if (!html) {
          throw new Error('No HTML content received from ScraperBee');
        }
